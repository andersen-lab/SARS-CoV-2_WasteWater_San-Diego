...
def test_correct_keys_in_config( seq_config ):
    for key in seq_config.keys():
        ...
        if "color" not in seq_config[key]:
            assert "parent" in seq_config[key], f"YAML entry {key} is incomplete. Must specify either a 'color' or 'parent' entry."
        else:
            assert "parent" not in seq_config[key], f"YAML entry {key} must contain either a 'color' and 'parent' entry, but not both."


def test_parent_in_config( seq_config ):
    for key in seq_config.keys():
        if 'parent' in seq_config[key]:
            assert seq_config[key]['parent'] in seq_config.keys(), f"YAML entry {key} specifies a parent ({seq_config[key]['parent']}) that does not exist."


def test_if_too_many_children( seq_config ):
    child_dict = dict()
    for key in seq_config.keys():
        if 'parent' in seq_config[key]:
            parent = seq_config[key]['parent']
            if parent in child_dict:
                child_dict[parent] += 1
            else:
                child_dict[parent] = 1
    max_children = int( 1 / ALPHA_DELTA )
    for parent, children in child_dict.items():
        assert children <= max_children, '{parent} has more children lineages ({children}) than the alpha value allows ({max_children}). Alpha decrease will pass 0.'
